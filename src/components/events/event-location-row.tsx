...
// the address, the map and the two ways into a maps app — one row of a panel rather than a card of
// its own, so the guest page and the host's event page each frame it once. every link is derived
// from the address itself, so a host never pastes one in and a corrected street can never leave a
// stale pin behind.
export function EventLocationRow({ address, className }: EventLocationRowProps) {
	...
	return (
		<div
			className={cn(
				'grid gap-4 sm:grid-cols-[minmax(0,13rem)_minmax(0,1fr)] sm:items-center sm:gap-6',
				className
			)}
		...
		>
			{coordinates ? (
				<EventLocationMap
					className='h-40 rounded-2xl sm:h-full sm:min-h-40'
